...
export class EmployeeDataService implements EmployeeDataInterface {
  ...
  getEmployeesList(formData: FormData): Observable<EmployeeListDataModel[]> {
    const empData = new EmployeeSearchDataModel();
    this.fmConverter.setProperty(formData, empData);
    const url = `${this.baseUrl}/employees/search`;
    return this.http.post<EmployeeListDataModel[]>(url, empData);
